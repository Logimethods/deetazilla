...
docker pull logimethods/int_cassandra_inject:1.5-dev
...
docker run --rm -v `pwd`/alt_properties:/templater/alt_properties logimethods/int_compose:1.5-dev combine_services -p alt_properties -e "local" "single" "no_secrets" \
